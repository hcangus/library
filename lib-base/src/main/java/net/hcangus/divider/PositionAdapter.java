...
class PositionAdapter {
	...
	public Position getPositionByIndex(int index) {
		...
		if (layoutManager instanceof GridLayoutManager) {
			...
			int spanSize = spanSizeLookup.getSpanSize(index - offset);
			int columns = spanSizeLookup.getSpanIndex(index - offset, numberOfColumns);
			int row = spanSizeLookup.getSpanGroupIndex(index - offset, numberOfColumns);
			int lastRow = spanSizeLookup.getSpanGroupIndex(getNumberOfItems() - 1, numberOfColumns);
			int lastCol = numberOfColumns - 1;
			for (int i = 0; i < numberOfColumns; i++) {
				lastCol -= (spanSizeLookup.getSpanSize(index - (columns - i) - offset) - 1);
			}
//			MyLogger.hcLog().i("index = " + index + "\n" +
//					"row = " + row + ", lastRow = " + lastRow + "\n" +
//					"col = " + columns + ", lastCol = " + lastCol);
			return new Position(index, columns, row, lastCol, lastRow);
		}
		return new Position(
				index,
	...
	private int getNumberOfColumns() {
		...
		if (layoutManager instanceof GridLayoutManager) {
			...
			numberOfColumns = manager.getSpanCount();
		}
