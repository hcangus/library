...
public class AdvancedPagerSlidingTabStrip extends HorizontalScrollView {
	...
	public interface IconTabProvider {
		<T> T getPageIcon(int position);

		<T> T getPageSelectIcon(int position);
...
//        txtParams.addRule(RelativeLayout.CENTER_IN_PARENT);
		...
		//16 new
		Drawable dot_drawable = ResourcesCompat.getDrawable(getResources(), R.drawable.shape_oval_tips, null);
	...
	public boolean setTabLayoutParams() {
		if (pager != null && pager.getAdapter() instanceof LayoutProvider) {
			...
			for (int i = 0; i < tabCount; i++) {
				...
				if (weight != 0.0f) {
					LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, weight);
