	...
	static void setStatusBarColorForCollapsingToolbar(Activity activity, final AppBarLayout appBarLayout, final CollapsingToolbarLayout collapsingToolbarLayout,
		...
		appBarLayout.addOnOffsetChangedListener(new AppBarLayout.OnOffsetChangedListener() {
			...
			public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
				final boolean[] isAnimToTrans = {false};
				if (Math.abs(verticalOffset) > appBarLayout.getHeight() - collapsingToolbarLayout.getScrimVisibleHeightTrigger()) {
...
//								.setDuration(collapsingToolbarLayout.getScrimAnimationDuration());
					if (window.getStatusBarColor() != Color.TRANSPARENT && !isAnimToTrans[0]) {
						ValueAnimator animator = ValueAnimator.ofArgb(window.getStatusBarColor(), Color.TRANSPARENT)
								.setDuration(collapsingToolbarLayout.getScrimAnimationDuration());
						animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
							@Override
							public void onAnimationUpdate(ValueAnimator valueAnimator) {
						...
						animator.addListener(new AnimatorListenerAdapter() {
							...
							public void onAnimationEnd(Animator animation) {
								isAnimToTrans[0] = false;
							}
						});
						isAnimToTrans[0] = true;
						animator.start();
					}
				}
			}
