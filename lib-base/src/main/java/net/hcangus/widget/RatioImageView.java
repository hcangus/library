...
public class RatioImageView extends android.support.v7.widget.AppCompatImageView {
	...
	// 宽高比例
	private float mWidthRatio = -1; // mWidthRatio = 宽度 / 高度 。
	private float mHeightRatio = -1; // mHeightRatio =  高度 / 宽度 。
	...
	public void setmIsWidthFitDrawableSizeRatio(boolean mIsWidthFitDrawableSizeRatio) {
		this.mIsWidthFitDrawableSizeRatio = mIsWidthFitDrawableSizeRatio;
	}

	public void setmIsHeightFitDrawableSizeRatio(boolean mIsHeightFitDrawableSizeRatio) {
		this.mIsHeightFitDrawableSizeRatio = mIsHeightFitDrawableSizeRatio;
	}

	public void setmWidthRatio(float mWidthRatio) {
		this.mWidthRatio = mWidthRatio;
	}

	public void setmHeightRatio(float mHeightRatio) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mDrawableSizeRatio > 0) {
			// 根据前景图宽高比例来测量view的大小
			if (mIsWidthFitDrawableSizeRatio && mWidthRatio < 0) {
				mWidthRatio = mDrawableSizeRatio;
			} else if (mIsHeightFitDrawableSizeRatio && mHeightRatio < 0) {
