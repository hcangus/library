...
public class SwipeMenuLayout extends ViewGroup {
	...
	//存储的是当前正在展开的View
	private static WeakReference<SwipeMenuLayout> mViewCache;
		...
		if (isSwipeEnable) {
			...
			switch (ev.getAction()) {
				case MotionEvent.ACTION_DOWN:
					...
					if (mViewCache != null) {
						if (mViewCache.get() != this) {
							mViewCache.get().smoothClose();
	...
	public void smoothExpand() {
        ...
        invalidate();*/
		//展开就加入ViewCache：
		mViewCache = new WeakReference<>(this);
...
/*        mScroller.startScroll(getScrollX(), 0, -getScrollX(), 0);
	...
	protected void onDetachedFromWindow() {
		if (mViewCache != null && this == mViewCache.get()) {
			mViewCache.get().smoothClose();
			mViewCache = null;
		}
...
/*    @Override
	...
	public void quickClose() {
		if (mViewCache != null && this == mViewCache.get()) {
			//先取消展开动画
			cancelAnim();
			mViewCache.get().scrollTo(0, 0);//关闭
			mViewCache = null;
		}
	}
