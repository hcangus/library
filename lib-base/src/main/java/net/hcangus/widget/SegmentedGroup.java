...
/**
 * app:sc_border_width="0dp"  边框宽度，边框颜色与tint_color一样
 * app:sc_tint_color="#ffffff"  被选中的背景色
 * app:sc_corner_radius="0dp"  边框弧度
 * app:sc_checked_text_color="@color/c_font1"  选中的字体颜色
 * android:orientation="horizontal"    选择方向
 * <p>
 * {@link #setGroupItems(List, int)}  设置各个选项
 * {@link #checkIndex(int)}   设置被选中的index
 * {@link #getCheckedIndex()} 获取被选中的index
 * {@link #setOnGroupItemCheckedListener(OnGroupItemCheckedListener)}  点击事件
 */
public class SegmentedGroup extends RadioGroup {
	...
	public enum DrawableLocation {
		...
		DrawableLocation(int id) {
			this.id = id;
		}

		static DrawableLocation fromId(int id) {
			for (DrawableLocation f : values()) {
				if (f.id == id)
					return f;
			}
			// default value
	...
	private SparseArray<TransitionDrawable> mDrawableMap;
	private int mLastCheckId;
	private DrawableLocation drawableLocation = DrawableLocation.Top;
	private OnGroupItemCheckedListener mGroupItemCheckedListener;

	public SegmentedGroup(Context context) {
		super(context);
		resources = getResources();
		mTintColor = resources.getColor(android.R.color.holo_blue_bright);
		mUnCheckedTintColor = resources.getColor(android.R.color.transparent);
		mMarginDp = (int) getResources().getDimension(R.dimen.line_1dp);
		mCornerRadius = getResources().getDimension(R.dimen.r_small);
	...
	private void initAttrs(AttributeSet attrs) {
		...
		try {
			...
			drawableLocation = DrawableLocation.fromId(typedArray.getInt(R.styleable.SegmentedGroup_sc_drawable_location, DrawableLocation.Top.id));
		} finally {
			typedArray.recycle();
		}
	}
	...
	private void updateBackground(View view) {
		...
		//Set text color
		int defaultColor = ((Button) view).getTextColors().getDefaultColor();
		ColorStateList colorStateList = new ColorStateList(new int[][]{
				{-android.R.attr.state_checked},
				{android.R.attr.state_checked}},
				new int[]{defaultColor, mCheckedTextColor});
	...
	public interface OnGroupItemCheckedListener {
		/**
		 * @param index 选择条目的索引
		 *              {@link SegmentedGroup.OnGroupItemCheckedListener}
		 * @see SegmentedGroup#setOnGroupItemCheckedListener(SegmentedGroup.OnGroupItemCheckedListener)
		 */
		void onItemChecked(int index);
	}

	public void setOnGroupItemCheckedListener(OnGroupItemCheckedListener listener) {
		this.mGroupItemCheckedListener = listener;
		if (listener != null) {
			setOnCheckedChangeListener(new OnCheckedChangeListener() {
				...
				public void onCheckedChanged(RadioGroup group, @IdRes int checkedId) {
					...
					for (int i = 0; i < childCount; i++) {
						...
						if (id == checkedId) {
							if (mGroupItemCheckedListener != null) {
								mGroupItemCheckedListener.onItemChecked(i);
							}
							break;
						}
					}
	 ...
	 *
	 * @param list   显示的数据源
	 * @param drawRes   图片的数据源
	 * @param itemLayoutId   显示的RadioButton的布局  R.layout.layout_radio_button
	 */
	public void setGroupItems(@NonNull List<String> list, @Nullable int[] drawRes, @LayoutRes int itemLayoutId) {
		if (drawRes != null && drawRes.length != list.size()) {
			throw new RuntimeException("标题和图片数量不等！");
		}
		int size = list.size();
		for (int i = 0; i < size; i++) {
			RadioButton radioButton = (RadioButton) LayoutInflater.from(getContext()).
					inflate(itemLayoutId, this, false);
			radioButton.setId(size + i);
			radioButton.setText(list.get(i));
			if (drawRes != null) {
				int[] res = new int[4];
				res[drawableLocation.id] = drawRes[i];
				radioButton.setCompoundDrawablesWithIntrinsicBounds(res[0], res[1], res[2], res[3]);
			}
			addView(radioButton);
		}
		updateBackground();
	...
	/**
	 * @see #setGroupItems(List, int[],int)
	 */
	public void setGroupItems(@ArrayRes int strArrRes,  @Nullable int[] drawRes, @LayoutRes int itemLayoutId) {
		String[] tables = getContext().getResources().getStringArray(strArrRes);
		setGroupItems(tables, drawRes, itemLayoutId);
	}

	/**
	 * @see #setGroupItems(List, int[],int)
	 */
	public void setGroupItems(@NonNull String[] strArr,  @Nullable int[] drawRes, @LayoutRes int itemLayoutId) {
		setGroupItems(Arrays.asList(strArr), drawRes, itemLayoutId);
	}

	/**
	 * @see #setGroupItems(List, int[], int)
	 */
	public void setGroupItems(@NonNull List<String> list, @LayoutRes int itemLayoutId) {
		setGroupItems(list, null, itemLayoutId);
	}


	/**
	 * @see #setGroupItems(List, int)
	 */
	public void setGroupItems(@ArrayRes int strArrRes, @LayoutRes int itemLayoutId) {
		String[] tables = getContext().getResources().getStringArray(strArrRes);
		setGroupItems(tables, itemLayoutId);
	}

	/**
	 * @see #setGroupItems(List, int)
	 */
	public void setGroupItems(@NonNull String[] strArr, @LayoutRes int itemLayoutId) {
		setGroupItems(Arrays.asList(strArr), itemLayoutId);
	}

	/**
	 * 设置默认选中项
	 *
	 * @param index 默认选中项的索引
	 */
	...
	private class LayoutSelector {
		...
		LayoutSelector(float cornerRadius) {
		...
		/* Returns the selected layout user_id based on view */
		int getSelected() {
			return SELECTED_LAYOUT;
		}

		/* Returns the unselected layout user_id based on view */
		int getUnselected() {
			return UNSELECTED_LAYOUT;
		}

		/* Returns the radii float table based on view for Gradient.setRadii()*/
		float[] getChildRadii(View view) {
