...
public class SimpleRatingBar extends View {
	...
	private void parseAttrs(AttributeSet attrs) {
		...
		borderColor = arr.getColor(R.styleable.SimpleRatingBar_srb_borderColor, getResources().getColor(R.color.c_bg_main));
	...
	private void validateAttrs() {
		if (numberOfStars <= 0) {
			throw new IllegalArgumentException(String.format(Locale.CHINA, "SimpleRatingBar initialized with invalid value " +
					"for numberOfStars. Found %d, but should be greater than 0", numberOfStars));
		}
		...
		if (stepSize <= 0) {
			throw new IllegalArgumentException(String.format(Locale.CHINA, "SimpleRatingBar initialized with invalid value " +
					"for stepSize. Found %f, but should be greater than 0", stepSize));
		}
		if (starBorderWidth <= 0) {
			throw new IllegalArgumentException(String.format(Locale.CHINA, "SimpleRatingBar initialized with invalid value " +
					"for starBorderWidth. Found %f, but should be greater than 0", starBorderWidth));
		}
		if (starCornerRadius < 0) {
			throw new IllegalArgumentException(String.format(Locale.CHINA, "SimpleRatingBar initialized with invalid value " +
					"for starCornerRadius. Found %f, but should be greater or equal than 0", starBorderWidth));
		}
	}
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
	...
	public void setStarBorderWidth(float starBorderWidth) {
		...
		if (starBorderWidth <= 0) {
			throw new IllegalArgumentException(String.format(Locale.CHINA, "SimpleRatingBar initialized with invalid value " +
							"for starBorderWidth. Found %f, but should be greater than 0",
					starBorderWidth));
		}
		paintStarBorder.setStrokeWidth(starBorderWidth);
		// request redraw of the view
	...
	public void setStarCornerRadius(float starCornerRadius) {
		...
		if (starCornerRadius < 0) {
			throw new IllegalArgumentException(String.format(Locale.CHINA, "SimpleRatingBar initialized with invalid value " +
							"for starCornerRadius. Found %f, but should be greater or equal than 0",
					starCornerRadius));
		}
		cornerPathEffect = new CornerPathEffect(starCornerRadius);
		paintStarBorder.setPathEffect(cornerPathEffect);
