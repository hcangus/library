...
import net.hcangus.banner.BannerConfig;
import net.hcangus.banner.BannerScroller;
import net.hcangus.banner.WeakHandler;
import net.hcangus.banner.listener.OnBannerListener;
...
public class Banner<T> extends FrameLayout implements OnPageChangeListener {
	...
	private int mIndicatorSelectedResId = R.drawable.shape_oval_white;
	private int mIndicatorUnselectedResId = R.drawable.shape_oval_black46;
	...
	private LinearLayout indicator, indicatorInside, indicatorBelow, titleView;
	private ImageLoaderInterface<T> imageLoader;
	...
	private void initView(Context context, AttributeSet attrs) {
		...
		indicatorBelow = (LinearLayout) view.findViewById(R.id.indicatorBelow);
	...
	private void handleTypedArray(Context context, AttributeSet attrs) {
		...
		mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.shape_oval_white);
		mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_unselected,
				R.drawable.shape_oval_black46);
	...
	public Banner<T> isAutoPlay(boolean isAutoPlay) {
		this.isAutoPlay = isAutoPlay;
		return this;
	}

	public Banner<T> setImageLoader(ImageLoaderInterface<T> imageLoader) {
		this.imageLoader = imageLoader;
		return this;
	}

	public Banner<T> setDelayTime(int delayTime) {
		this.delayTime = delayTime;
		return this;
	}

	public Banner<T> setIndicatorGravity(int type) {
		switch (type) {
	...
	public Banner<T> setBannerAnimation(Class<? extends PageTransformer> transformer) {
		try {
	 ...
	 * @return Banner
	 */
	public Banner<T> setOffscreenPageLimit(int limit) {
		if (viewPager != null) {
	 ...
	 * @return Banner
	 */
	public Banner<T> setPageTransformer(boolean reverseDrawingOrder, PageTransformer transformer) {
		viewPager.setPageTransformer(reverseDrawingOrder, transformer);
		return this;
	}

	public Banner<T> setBannerTitles(List<String> titles) {
		this.titles = titles;
		return this;
	}

	public Banner<T> setBannerStyle(int bannerStyle) {
		this.bannerStyle = bannerStyle;
		return this;
	}

	public Banner<T> setViewPagerIsScroll(boolean isScroll) {
		this.isScroll = isScroll;
		return this;
	}

	public Banner<T> setImages(List<T> imageUrls) {
	...
	public void updateBannerStyle(int bannerStyle) {
		...
		indicatorBelow.setVisibility(GONE);
	...
	private void setBannerStyleUI() {
		...
		switch (bannerStyle) {
			case BannerConfig.CIRCLE_INDICATOR:
				...
				break;
			case BannerConfig.CIRCLE_INDICATOR_BELOW:
				indicatorBelow.setVisibility(visibility);
				break;
	...
	private void initImages() {
		...
		if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
				...
				bannerStyle == BannerConfig.CIRCLE_INDICATOR_BELOW) {
			createIndicator();
		} else if (bannerStyle == BannerConfig.NUM_INDICATOR_TITLE) {
			numIndicatorInside.setText(String.valueOf("1/" + count));
		} else if (bannerStyle == BannerConfig.NUM_INDICATOR) {
			numIndicator.setText(String.valueOf("1/" + count));
		}
	}

	private void setImageList(List<T> imagesUrl) {
		...
		for (int i = 0; i <= count + 1; i++) {
			View view = imageLoader.createView(context);
			setScaleType(imageLoader.getImageView());
			T url;
			if (i == 0) {
			...
			viewList.add(view);
			imageLoader.displayImage(context, url, imageLoader.getImageView());
		}
	}
	...
	private void createIndicator() {
		...
		for (int i = 0; i < count; i++) {
			...
			else if (bannerStyle == BannerConfig.CIRCLE_INDICATOR_BELOW)
				indicatorBelow.addView(imageView, params);
		}
	}
